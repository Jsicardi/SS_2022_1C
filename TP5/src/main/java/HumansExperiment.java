    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int Nh : humans){
            for(int i = 1; i <= 10; i++){
                ...
                ZombieDynamics.main(new String[]{"static_rand.txt", "dynamic_rand.txt", String.format("result_%d_%d.txt",Nh,i),Double.toString(vdh), Double.toString(vdz), Double.toString(Ap), Double.toString(Bp), Double.toString(savingT), Double.toString(transformationT), Double.toString(tf), Double.toString(beta), Double.toString(cureProbability), Boolean.toString(withEnergy)});
